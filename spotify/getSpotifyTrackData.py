...
def create_big_query_table(table_id, schema, client):
    ...
    print(
        f"Table '{table_id.table_id}' created successfully in dataset '{table_id.dataset_id}'.")
...
def transform_spotify_track_data(data, current_datetime):
    ...
    better_column_names = {col: col.replace(
        '.', '_') for col in flattened_data.columns}
...
def trackData_handler(event, context):
    ...
    bigquery_schema = [SchemaField(field['name'], field['type'])
    ...
    if table_exists(bq_table_reference, bigquery_client):
        ...
        create_big_query_table(
            bq_table_reference, bigquery_schema, bigquery_client)

    # get access token using your refresh token
    try:
        token = refresh_access_token(
            client_id, client_secret, refresh_token, authorization_code)
    except Exception as e:
    ...
    try:
        transformed_data = transform_spotify_track_data(
            trackData, current_datetime)
    except Exception as e:
    ...
    try:
        for field in table_schema:
            ...
            if data_type == 'INTEGER':
                transformed_data[col_name] = pd.to_numeric(
                    transformed_data[col_name], errors='coerce').astype('Int64')
        ...
        transformed_data = transformed_data.where(
            pd.notnull(transformed_data), None)

    except Exception as e:
        return send_response(500, 'Failed to apply data validation.', e)

    transformed_data = transformed_data[transformed_data['played_at']
                                        > last_queried_from]
